...
const Header = () => {
  ...
  const user = { name: "chester", email: "conyasetia@gmail.com" };
  const logout = () => {};
      ...
      <div className="max-x-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 text-blue-900 rounded-md flex items-center justify-center">
          ...
          <div className="hidden lg:flex items-center  space-x-4">
            {isAuthenticated ? (
             ...
             avatat={user?.avatar || ""}  companyName={user?.name || ""} email={user?.email || ""} onLogOut={logout} />
            ) : (
              <>
                <Link
                  to="/login"
                ...
                <Link
                  to="/signup"
                  className="bg-gradient-to-r from-blue-950 to-blue-600 hover:bg-gray-600 text-white px-6 py-2.5 rounded-lg font-medium transition-all duration-200 hover:scale-105 hover:shadow-lg"
                >
                  Sign Up
                </Link>
              </>
          ...
          <div className="lg:hidden">
            <button
              ...
              className="p-2 rounded-lg hover:text-gray-900 hover:bg-gray-200 transition-colors duration-200"
            >
              {isMenuOpen ? (
                <X className="w-6 h-6" />
              ) : (
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-lg">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <a href="#features" className="block px-4 py-3 space-y-1 hover:text-gray-600 hover:bg-gray-50 font-medium transition-colors duration-200">Features</a>
            <a href="#testimonials" className="block px-4 py-3 space-y-1 hover:text-gray-600 hover:bg-gray-50 font-medium transition-colors duration-200">Testimonials</a>
            <a href="#faqs" className="block px-4 py-3 space-y-1 hover:text-gray-600 hover:bg-gray-50 font-medium transition-colors duration-200">FAQs</a>
